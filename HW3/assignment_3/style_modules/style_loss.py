...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        H = features.shape[2]
        W = features.shape[3]
        flattened = features.view(N, C, -1)

        matrix_mult = torch.bmm(flattened, flattened.transpose(1, 2))
        if normalize:
          matrix_mult /= (C * H * W)
        return matrix_mult
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        for i, layer in enumerate(style_layers):
          distance = torch.square(self.gram_matrix(feats[layer]) - style_targets[i])
          loss += style_weights[i] * torch.sum(distance)
        return loss
