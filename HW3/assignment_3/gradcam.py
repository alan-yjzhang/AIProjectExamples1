...
visualize_attr_maps('visualization/GuidedGradCam.png', X, y, class_names, [attr_ggc], ['Guided GradCam'])

# Computing Guided BackProp
gp_grads = GuidedBackprop(model)
attr_gp = compute_attributions(gp_grads, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/GuidedBackprop.png', X, y, class_names, [attr_gp], ['Guided Backprop'])
...
layer_lgc = LayerGradCam(model, layer)
layer_lgc_attr = compute_attributions(layer_lgc, X_tensor, target=y_tensor, relu_attributions=True)
layer_lgc_attr_sum = layer_lgc_attr.mean(axis=1, keepdim=True)

layer_lc = LayerConductance(model, layer)
layer_lc_attr = compute_attributions(layer_lc, X_tensor, target=y_tensor)
layer_lc_attr_sum = layer_lc_attr.mean(axis=1, keepdim=True)

visualize_attr_maps('visualization/LayerActivation.png', X, y, class_names, [layer_act_attr_sum], ['LayerActivation'])
visualize_attr_maps('visualization/LayerGradCam.png', X, y, class_names, [layer_lgc_attr_sum], ['LayerGradCam'])
visualize_attr_maps('visualization/LayerConductance.png', X, y, class_names, [layer_lc_attr_sum], ['LayerConductance'])
